...
(test test-array-bind
  ...
  (is (subtypep (rtl:typecheck '(let ((a (make-array '(16)
					  :element-type (unsigned-byte 8)))
				      (b 0))
				 (setq b 12))
			       emptyenv)
...
(test test-synthesise-array-decl
  ...
  (is (rtl:synthesise '(let ((a (make-array '(16)
				 :element-type (unsigned-byte 8)))
			     (b (make-array '(8)
				 :element-type (unsigned-byte 8))
			      :as :wire)
...
(test test-synthesise-array-init-from-data
  ...
  (is (rtl:synthesise '(let ((b (make-array '(4)
				 :element-type (unsigned-byte 8)
...
(test test-syntheseise-array-init
  ...
  (is (rtl:synthesise '(let ((a (make-array '(10)
